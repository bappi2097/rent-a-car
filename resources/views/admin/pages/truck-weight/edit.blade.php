...
@section('content')
    <a href="{{ route('admin.car-weight-category.index') }}" class="btn btn-white"> &lt; Back</a>
    <div class="bg-white p-20 col-12 m-t-30">
        <form action="{{ route('admin.car-weight-category.update', $carWeightCategory->id) }}" method="POST"
            enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <fieldset>
                <legend class="m-b-15">Edit Car Weight Category</legend>
                <div class="form-group">
                    <label for="name">Name</label>
                    <input type="text" class="form-control" name="name" id="name" placeholder="Seven and Half Ton"
                        value="{{ $carWeightCategory->name }}">
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <input type="number" class="form-control" name="weight" id="weight" placeholder="7.5" min="1" max="60"
                        value="{{ $carWeightCategory->weight }}">
                    @error('weight')
                        <span class="text-red">{{ $message }}</span>
                    @enderror
                </div>
