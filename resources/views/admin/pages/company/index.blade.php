                    ...
                    @foreach ($companies as $index => $company)
                        <tr>
                            ...
                            <td class="with-btn" nowrap="">
                                <a href="{{ route('admin.user.company.car.index', $company->company->id) }}"
                                    class="btn btn-sm btn-warning width-60 m-r-2">car</a>
                                ...
                                <form id="language{{ $index }}"
                                    action="{{ route('admin.user.company.destroy', $company->id) }}" method="POST"
                                    style="display: none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
